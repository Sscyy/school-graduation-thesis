"""HoverNet inference and evaluation on PanNuke fold3.

只评估实例分割性能（DICE / AJI / DQ / SQ / PQ），不涉及核类型分类。
...
def model_out_to_pred_map(out: dict) -> np.ndarray:
    """不做分类时，pred_map shape = (H, W, 3) = [np_prob, hv_x, hv_y]。"""
    np_prob = F.softmax(out["np"], dim=1)[0, 1].cpu().numpy()
    hv      = out["hv"][0].cpu().numpy().transpose(1, 2, 0)
    return np.dstack([np_prob, hv])
...
def main():
    ...
    model = HoVerNet(input_ch=3, nr_types=None, freeze=False, mode=args.mode)
